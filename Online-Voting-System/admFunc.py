...
def showVotes(root,frame1):
    ...
    bjpLogo = ImageTk.PhotoImage((Image.open("img/bjp.png")).resize((35,35),PIL.Image.Resampling.LANCZOS))
    bjpImg = Label(frame1, image=bjpLogo).grid(row = 2,column = 0)

    congLogo = ImageTk.PhotoImage((Image.open("img/cong.jpg")).resize((25,38),PIL.Image.Resampling.LANCZOS))
    congImg = Label(frame1, image=congLogo).grid(row = 3,column = 0)

    aapLogo = ImageTk.PhotoImage((Image.open("img/aap.png")).resize((45,30),PIL.Image.Resampling.LANCZOS))
